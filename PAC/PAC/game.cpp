...
Game::Game()
{
    ...
    gamewon.load("C:/Users/3mrr3/OneDrive/Desktop/AUC/Fall 2023/Applied Data Structures/PAC/Objects/gamewon.jpg");
    ...
    connect(releaseTimer,&QTimer::timeout, this,&Game::releaseGhosts);

    connect(spawnTimer,&QTimer::timeout, this,&Game::spawnPowerUp);

    connect(Gho1->chaseTimer, &QTimer::timeout, [this]() {chasePacman(Gho1,Pac->xcor,Pac->ycor);});
    connect(Gho2->chaseTimer, &QTimer::timeout, [this]() {chasePacman(Gho2,Pac->xcor+5,Pac->ycor);});
    connect(Gho3->chaseTimer, &QTimer::timeout, [this]() {chasePacman(Gho3,Pac->xcor-5,Pac->ycor);});
    connect(Gho4->chaseTimer, &QTimer::timeout, [this]() {chasePacman(Gho4,Pac->xcor,Pac->ycor+5);});
...
bool Game::gameWon()
{
    for(int i=0;i<MapRows;i++)
    {
        for(int j=0;j<MapCols;j++)
        {
            if(gameMap[i][j]->objectType== type::Ball || gameMap[i][j]->objectType== type::PowerBall)
            {
                return false;
            }
        }
...
void Game::collision()
{
    ...
    else if((Pac->collidesWithItem(Gho1))||(Pac->collidesWithItem(Gho2))||(Pac->collidesWithItem(Gho3))||(Pac->collidesWithItem(Gho4)))
    {
        ...
        else if((Gho1->condition==ghostCondition::Normal && Pac->collidesWithItem(Gho1)) || (Gho2->condition==ghostCondition::Normal && Pac->collidesWithItem(Gho2)) || (Gho3->condition==ghostCondition::Normal && Pac->collidesWithItem(Gho3)) || (Gho4->condition==ghostCondition::Normal && Pac->collidesWithItem(Gho4)))
        {
            Pac->lives--;
            Pac->xcor=Pac->initx;
            Pac->ycor=Pac->inity;
